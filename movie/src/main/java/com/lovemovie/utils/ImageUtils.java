...
public class ImageUtils {
    ...
    public static String uploadHeadImg(HttpServletRequest request,MultipartFile picture) throws Exception {
        //定义图片路径
        String imgPath=null;
        ...
        if (!picture.isEmpty()&&picture!=null){
            ...
            String ext= FilenameUtils.getExtension(picture.getOriginalFilename());
            //设置图片上传路径
            String url = request.getSession().getServletContext().getRealPath("/upload/head");
            //判断文件夹是否存在
            File file = new File(url);
            if (!file.exists()){
                file.mkdirs();
    ...
    public static String uploadActorHeadImg(HttpServletRequest request,MultipartFile picture) throws Exception {
        //定义图片路径
        String imgPath=null;
        ...
        if (!picture.isEmpty()&&picture!=null){
            ...
            String ext= FilenameUtils.getExtension(picture.getOriginalFilename());
            //设置图片上传路径
            String url = request.getSession().getServletContext().getRealPath("/upload/actor");
            //判断文件夹是否存在
            File file = new File(url);
            if (!file.exists()){
                file.mkdirs();
