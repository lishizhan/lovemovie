                        ...
                        <div class="pull-left">
                            <div class="form-group form-inline">
                                ...
                                <div class="input-group input-group-sm">
                                    <input id="searchMsg" type="text" class="form-control" placeholder="请输入用户账号进行查询">
...
<script>
    ...
    function toPage(pageNum, pageSize,userName) {
        $.ajax({
            url: "management/getAllOrderInfo",
            data: {
                "pageNum": pageNum,
                "pageSize": pageSize,
                "userName":userName
            ...
            success: function (res) {
                ...
                console.log(res);
                //1，展示电影数据
                build_order_info_table(res);
                //2,解析并显示分页信息
                build_order_info_info(res);
                //3,解析分页条数据
                build_order_info_nav(res);
    ...
    //显示页码，总页码等
    function build_order_info_info(res) {
        $("#page_info").empty();
    ...
    function build_order_info_table(res) {
        //清空表格
        $("#dataList tbody").empty();

        let orders = res.extend.pageInfo.list;

        if (orders.length===0){
            let msg = $("<td colspan='11'></td>").append("<h4>查询不到指定的数据，请点击刷新~</h4>");
        ...
        $.each(orders, function (index, item) {
            ...
            let orderId = $("<td></td>").append(item.orderId);
            let orderUser = $("<td></td>").append(item.orderUser.userName);

            let scheduleStarttime = $("<td></td>").append(timeFormatSeconds(item.orderSchedule.scheduleStarttime));
            let orderPosition = $("<td></td>").append(item.orderPosition);
            let schedulePrice = $("<td></td>").append(item.orderSchedule.schedulePrice);
            let  movieCnName = $("<td></td>").append(item.orderSchedule.scheduleMovie.movieCnName);
            let hallName = $("<td></td>").append(item.orderSchedule.scheduleHall.hallName);
            let cinemaName = $("<td></td>").append(item.orderSchedule.scheduleHall.hallCinema.cinemaName);
            //订单状态 0:退票中 1:已支付 2:退票成功
            let orderState = $("<td></td>");
            if (item.orderState === 1) {
                orderState.append("已支付");
            } else if (item.orderState===0){
                orderState.append("退票中");
            }else {
                orderState.append("退票成功");
            }
            // 根据毫秒数构建 Date 对象
            ...
            deleteBtn.attr("del-id", item.orderId);
            /*var queryBtn = $("<button></button>")
                .css("marginLeft", "20px")
                .addClass("btn bg-aqua btn-xs query_btn").text("详情/编辑");*/
            // queryBtn.attr("query-id", item.movieId);.append(queryBtn)
    ...
    //生成分页按钮
    function build_order_info_nav(res) {
        $("#page_nav").empty();
    ...
    $(document).on("click",".delete_btn",function () {
        ...
        let orderId = $(this).attr("del-id");
        if (confirm("是否删除订单编号为《"+name+"》？")){
            //请求后台删除
            $.ajax({
                url:"management/deleteOrder/"+orderId,
                type:"delete",
                success:function (res) {
                    console.log(res);
                    if (res.code===100){
    ...
    $("#delBtn").click(function () {
        ...
        $.each($(".check_item:checked"),function () {
            ...
            orderId+=$(this).parents("tr").find("td:eq(1)").text()+"-"
        });
        empNames=empNames.substring(0,empNames.length-1);
        orderId=orderId.substring(0,orderId.length-1);
        if (confirm("是否删除【"+empNames+"】等订单？")){
